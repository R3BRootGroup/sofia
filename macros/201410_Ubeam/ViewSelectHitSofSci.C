...
void ViewSelectHitSofSci(UInt_t FirstRun = 1355)
{
    ...
    h2_TofRaw_vs_PosRaw[1] =
        new TH2D("CaveC_TofRaw_vs_DeltaT", "CaveC_TofRaw_vs_DeltaT", 3000, 19, 22, 3000, 1704, 1719);
    ...
    for (int Event = 0; Event < nevents; Event++)
    {
        ...
        Double_t iTns;
        Double_t Tns[6];
        for (UShort_t sig = 0; sig < 6; sig++)
            Tns[sig] = -1;
        nHits_tcalSofSci = tcalSofSci->GetEntries();
        if (nHits_tcalSofSci == 6)
        { // SELECT MULT=1
            ...
            if ((Tns[0] != -1) && (Tns[1] != -1) && (Tns[2] != -1) && (Tns[3] != -1) && (Tns[4] != -1) &&
                (Tns[5] != -1))
            {
                DeltaT[0] = Tns[2] - Tns[1];
                h1_tcalSofSci_RawPos[0]->Fill(DeltaT[0]);
                DeltaT[1] = Tns[5] - Tns[4];
                h1_tcalSofSci_RawPos[1]->Fill(DeltaT[1]);
    ...
    can_SofSci_RawPos->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_tcalSofSci_RawPos[0]->SetLineColor(kBlue + 2);
    h1_tcalSofSci_RawPos[0]->Draw();
    can_SofSci_RawPos->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_tcalSofSci_RawPos[1]->SetLineColor(kBlue + 2);
    h1_tcalSofSci_RawPos[1]->Draw();
    can_SofSci_RawPos->cd(3);
    gPad->SetGridx();
    gPad->SetGridy();
    h2_tcalSofSci_RawPos_S2vsCC->Draw("COL");

    TCanvas* can_SofSci_TofRaw = new TCanvas("RawTof_SofSci", "RawTof_SofSci", 0, 0, 1200, 1200);
    can_SofSci_TofRaw->Divide(2, 2);
    can_SofSci_TofRaw->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_TofRaw->SetLineColor(kBlue + 2);
    h1_TofRaw->Draw();
    can_SofSci_TofRaw->cd(3);
    gPad->SetGridx();
    gPad->SetGridy();
    h2_TofRaw_vs_PosRaw[0]->Draw("COL");
    can_SofSci_TofRaw->cd(4);
    gPad->SetGridx();
    gPad->SetGridy();
    h2_TofRaw_vs_PosRaw[1]->Draw("COL");

    TH1D* h1_TofRaw_Integral =
        new TH1D("TofRaw_IntegralPerBin",
                 "TofRaw_IntegralPerBin",
                 h1_TofRaw->GetNbinsX(),
                 h1_TofRaw->GetBinLowEdge(1),
                 h1_TofRaw->GetBinLowEdge(h1_TofRaw->GetNbinsX()) + h1_TofRaw->GetBinWidth(h1_TofRaw->GetNbinsX()));
    for (UShort_t bin = 1; bin <= h1_TofRaw->GetNbinsX(); bin++)
    {
        h1_TofRaw_Integral->SetBinContent(bin, h1_TofRaw->Integral(1, bin));
    }
    can_SofSci_TofRaw->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_TofRaw_Integral->SetLineColor(kBlue + 2);
    h1_TofRaw_Integral->Draw();
