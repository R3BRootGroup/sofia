...
void ViewVftx()
{
    ...
    for (int Event = 0; Event < nevents; Event++)
    {
        ...
        if (nHits_mappedSofComRef != 0)
        {
            ...
            for (Int_t hit = 0; hit < nHits_mappedSofComRef; hit++)
            {
                ...
                h1_mappedSofComfRef_ft[evt_mappedSofComRef[hit]->GetDetector() - 1]->Fill(
                    evt_mappedSofComRef[hit]->GetTimeFine());
                h1_mappedSofComfRef_ct[evt_mappedSofComRef[hit]->GetDetector() - 1]->Fill(
                    evt_mappedSofComRef[hit]->GetTimeCoarse());
            }
            if (nHits_mappedSofComRef == 2)
                h1_mappedSofComRef_DELTAct->Fill(evt_mappedSofComRef[0]->GetTimeCoarse() -
                                                 evt_mappedSofComRef[1]->GetTimeCoarse());
        }
#endif
#if SOF_SCI
        ...
        if (nHits_mappedSofSci != 0)
        {
            ...
            for (Int_t hit = 0; hit < nHits_mappedSofSci; hit++)
            {
                ...
                h1_mappedSofSci_ft[evt_mappedSofSci[hit]->GetDetector() - 1][evt_mappedSofSci[hit]->GetPmt() - 1]->Fill(
                    evt_mappedSofSci[hit]->GetTimeFine());
                h1_mappedSofSci_ct[evt_mappedSofSci[hit]->GetDetector() - 1][evt_mappedSofSci[hit]->GetPmt() - 1]->Fill(
                    evt_mappedSofSci[hit]->GetTimeCoarse());
                if ((evt_mappedSofSci[hit]->GetDetector() == 1) && (evt_mappedSofSci[hit]->GetPmt() == 1))
                    ctDet1Pmt1 = evt_mappedSofSci[hit]->GetTimeCoarse();
                if ((evt_mappedSofSci[hit]->GetDetector() == 1) && (evt_mappedSofSci[hit]->GetPmt() == 2))
                    ctDet1Pmt2 = evt_mappedSofSci[hit]->GetTimeCoarse();
                if ((evt_mappedSofSci[hit]->GetDetector() == 2) && (evt_mappedSofSci[hit]->GetPmt() == 1))
                    ctDet2Pmt1 = evt_mappedSofSci[hit]->GetTimeCoarse();
                if ((evt_mappedSofSci[hit]->GetDetector() == 2) && (evt_mappedSofSci[hit]->GetPmt() == 2))
                    ctDet2Pmt2 = evt_mappedSofSci[hit]->GetTimeCoarse();
            }
            if ((nHits_mappedSofSci == 4) && (ctDet1Pmt1 != -1) && (ctDet1Pmt2 != -1) && (ctDet2Pmt1 != -1) &&
                (ctDet2Pmt2 != -1))
            {
        ...
        if (nHits_mappedSofToFW != 0)
        {
            ...
            for (Int_t hit = 0; hit < nHits_mappedSofToFW; hit++)
            {
                ...
                h1_mappedSofToFW_ft[evt_mappedSofToFW[hit]->GetDetector() - 1][evt_mappedSofToFW[hit]->GetPmt() - 1]
                    ->Fill(evt_mappedSofToFW[hit]->GetTimeFine());
            }
        }
#endif
#if SOF_TRIM
        ...
        if (nHits_mappedSofTrim != 0)
        {
            ...
            for (Int_t hit = 0; hit < nHits_mappedSofTrim; hit++)
            {
                ...
                h1_mappedSofTrim_ft[evt_mappedSofTrim[hit]->GetPlane() - 1][evt_mappedSofTrim[hit]->GetSection() - 1]
                                   [evt_mappedSofTrim[hit]->GetAnode() - 1]
                                       ->Fill(evt_mappedSofTrim[hit]->GetTimeFine());
            }
        }
#endif
#if SOF_TWIM
        ...
        if (nHits_mappedSofTwim != 0)
        {
            ...
            for (Int_t hit = 0; hit < nHits_mappedSofTwim; hit++)
            {
                ...
                h1_mappedSofTwim_ft[evt_mappedSofTwim[hit]->GetPlane() - 1][evt_mappedSofTwim[hit]->GetSection() - 1]
                                   [evt_mappedSofTwim[hit]->GetAnode() - 1]
                                       ->Fill(evt_mappedSofTwim[hit]->GetTimeFine());
            }
        }
#endif
    ...
    can_SofComRef_ft->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofComfRef_ft[0]->SetLineColor(kBlue + 2);
    h1_mappedSofComfRef_ft[0]->Draw();
    can_SofComRef_ft->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofComfRef_ft[1]->SetLineColor(kBlue + 2);
    h1_mappedSofComfRef_ft[1]->Draw();
    TCanvas* can_SofComRef_ct = new TCanvas("CoarseTime_SofComRef", "CoarseTime_SofComRef", 0, 0, 1200, 400);
    can_SofComRef_ct->Divide(3, 1);
    can_SofComRef_ct->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofComfRef_ct[0]->SetLineColor(kBlue + 2);
    h1_mappedSofComfRef_ct[0]->Draw();
    can_SofComRef_ct->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofComfRef_ct[1]->SetLineColor(kBlue + 2);
    h1_mappedSofComfRef_ct[1]->Draw();
    can_SofComRef_ct->cd(3);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofComRef_DELTAct->SetLineColor(kBlue + 2);
    h1_mappedSofComRef_DELTAct->Draw();
#endif
#if SOF_SCI
    ...
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ft[0][0]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ft[0][0]->Draw();
    can_SofSci_ft->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ft[0][1]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ft[0][1]->Draw();
    can_SofSci_ft->cd(3);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ft[1][0]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ft[1][0]->Draw();
    can_SofSci_ft->cd(4);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ft[1][1]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ft[1][1]->Draw();
    TCanvas* can_SofSci_ct = new TCanvas("CoarseTime_SofSci", "CoarseTime_SofSci", 0, 0, 1000, 1000);
    can_SofSci_ct->Divide(2, 2);
    can_SofSci_ct->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ct[0][0]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ct[0][0]->Draw();
    can_SofSci_ct->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ct[0][1]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ct[0][1]->Draw();
    can_SofSci_ct->cd(3);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ct[1][0]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ct[1][0]->Draw();
    can_SofSci_ct->cd(4);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_ct[1][1]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_ct[1][1]->Draw();
    TCanvas* can_SofSci_DeltaCt = new TCanvas("DeltaCt_SofSci", "DeltaCt_SofSci", 0, 0, 1000, 500);
    can_SofSci_DeltaCt->Divide(2, 1);
    can_SofSci_DeltaCt->cd(1);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_DELTAct[0]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_DELTAct[0]->Draw();
    can_SofSci_DeltaCt->cd(2);
    gPad->SetGridx();
    gPad->SetGridy();
    h1_mappedSofSci_DELTAct[1]->SetLineColor(kBlue + 2);
    h1_mappedSofSci_DELTAct[1]->Draw();
#endif
#if SOF_TOFW
    TCanvas* can_ToFW_ft[2];
    can_ToFW_ft[0] = new TCanvas("FineTime_ToFW_Pmt1Down", "FineTime_ToFW_Pmt1Down", 0, 0, 1500, 1400);
    can_ToFW_ft[0]->Divide(7, 4);
    can_ToFW_ft[1] = new TCanvas("FineTime_ToFW_Pmt2Up", "FineTime_ToFW_Pmt2Up", 0, 0, 1500, 1400);
    can_ToFW_ft[1]->Divide(7, 4);
    for (UShort_t det = 0; det < NUM_SOFTOFW_DETECTORS; det++)
    {
        can_ToFW_ft[0]->cd(det + 1);
        gPad->SetGridx();
        gPad->SetGridy();
        h1_mappedSofToFW_ft[det][0]->SetLineColor(kBlue + 2);
        h1_mappedSofToFW_ft[det][0]->Draw();
        can_ToFW_ft[1]->cd(det + 1);
        gPad->SetGridx();
        gPad->SetGridy();
        h1_mappedSofToFW_ft[det][1]->SetLineColor(kBlue + 2);
        h1_mappedSofToFW_ft[det][1]->Draw();
    }
#endif
#if SOF_TRIM
    ...
    for (UShort_t plane = 0; plane < NUM_SOFTRIM_PLANES; plane++)
    {
        for (UShort_t section = 0; section < NUM_SOFTRIM_SECTIONS; section++)
        {
            for (UShort_t anode = 0; anode < NUM_SOFTRIM_ANODES; anode++)
            {
                can_Trim_ft->cd(plane * NUM_SOFTRIM_SECTIONS * NUM_SOFTRIM_ANODES + section * NUM_SOFTRIM_ANODES +
                                anode + 1);
                gPad->SetGridx();
                gPad->SetGridy();
