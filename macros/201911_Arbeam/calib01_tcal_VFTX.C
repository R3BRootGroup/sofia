...
void calib01_tcal_VFTX(Int_t First = 1320)
{
    ...
    TString outputFileName = "../SofMacrosOutput/201911_online/calib_output_tcal_VFTX_All.root";
    TString outputFileNamePar = "../SofMacrosOutput/201911_online/Parameters/tcal_VFTX_All";
    ...
    if (rtdb->getCurrentRun())
        cout << "have run" << endl;
    else
        cout << "have no run" << endl;
    rtdb->saveOutput();
    /* ------------------------------------------------------ */
    ...
    for (int pmt = 0; pmt < tofwTcalibrator->GetNumChannels(); pmt++)
    {
        ...
        for (int plastic = 0; plastic < tofwTcalibrator->GetNumDetectors(); plastic++)
        {
            ...
            if (!h1_softofw_ftns[signal])
                continue;
            can_SofToFW[pmt]->cd(plastic + 1);
            gPad->SetGridx();
            gPad->SetGridy();
