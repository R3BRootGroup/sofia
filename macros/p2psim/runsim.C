...
void runsim(Int_t nEvents = 0)
{
    ...
    TString fTwimGeo = "twinmusic_v22.geo.root";
    ...
    if (fMwpc0) {
        if (mwpc0Par) {
            ...
            run->AddModule(
                new R3BMwpc0(fMwpc0Geo, {mwpc0Par->GetPosX(), mwpc0Par->GetPosY(), mwpc0Par->GetPosZ(), rmwpc0}));
        } else
    ...
    if (fCalifa) {
        if (califaPar) {
            ...
            R3BCalifa* califa =
                new R3BCalifa(fCalifaGeo, {califaPar->GetPosX(), califaPar->GetPosY(), califaPar->GetPosZ(), rcalifa});
    ...
    if (fMwpc1) {
        if (mwpc1Par) {
            ...
            run->AddModule(
                new R3BMwpc1(fMwpc1Geo, {mwpc1Par->GetPosX(), mwpc1Par->GetPosY(), mwpc1Par->GetPosZ(), rmwpc1}));
        } else
            run->AddModule(new R3BMwpc1(fMwpc1Geo, {0., 0., 42.}));
    ...
    if (fTwim) {
        if (twimPar) {
            ...
            run->AddModule(new R3BTwim(fTwimGeo, {twimPar->GetPosX(), twimPar->GetPosY(), twimPar->GetPosZ(), rtwim}));
        } else
            run->AddModule(new R3BTwim(fTwimGeo, {0., 0., 69.}));
    ...
    if (fMwpc2) {
        if (mwpc2Par) {
            ...
            run->AddModule(
                new R3BMwpc2(fMwpc2Geo, {mwpc2Par->GetPosX(), mwpc2Par->GetPosY(), mwpc2Par->GetPosZ(), rmwpc2}));
        } else
            run->AddModule(new R3BMwpc2(fMwpc2Geo, {0., 0., 100.}));
        auto mw2_digitizer = new R3BMwpcDigitizer("Mwpc2", 1);
        if (fSofiaDigitizer)
    ...
    if (fMwpc3) {
        ...
        if (mwpc3Par) {
            ...
            run->AddModule(
                new R3BMwpc3(fMwpc3Geo, {mwpc3Par->GetPosX(), mwpc3Par->GetPosY(), mwpc3Par->GetPosZ(), rmwpc3}));
        } else {
            rmwpc3->RotateY(-18.);
            run->AddModule(new R3BMwpc3(fMwpc3Geo, {-300., 0., 749., rmwpc3}));
        }
        auto mw3_digitizer = new R3BMwpcDigitizer("Mwpc3", 1);
        if (fSofiaDigitizer)
    ...
    if (fSofTofWall) {
        ...
        auto tofw_digitizer = new R3BSofTofWDigitizer();
        if (fSofiaDigitizer)
