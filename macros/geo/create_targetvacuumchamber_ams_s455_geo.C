...
void create_targetvacuumchamber_ams_geo(const char* geoTag="s455")
{
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
  ...
  TString geoFileName = geoPath + "/sofia/geometry/targetvacuumchamber_ams_";
  ...
  if ( ! pMed5 ) Fatal("Main", "Medium H2 not found");
...
/*
  TGeoShape *pbox1 = new TGeoBBox("amsbox1", 11/2.0, 11./2.0, 12./2.0);
  TGeoCombiTrans *t_box1 = new TGeoCombiTrans("t_box1",-12.,0.,12.,fRefRot);
  t_box1->RegisterYourself();

  TGeoShape *pbox2 = new TGeoBBox("amsbox2", 11/2.0, 11./2.0, 12./2.0);
  TGeoCombiTrans *t_box2 = new TGeoCombiTrans("t_box2",12.,0.,12.,fRefRot);
  t_box2->RegisterYourself();
  TGeoCompositeShape *pVCWorld = new TGeoCompositeShape("VCbox", "CenterPart:t_tube + Cone:t_cone - (amsbox1:t_box1 + amsbox2:t_box2)");
*/
...
*/
  ...
  // Si strip Shape & volume: TGeoBBox
  dx = 0.0110;
  dy = 3.9936;
  dz = 0.0150;
  TGeoVolume *StripS = gGeoManager->MakeBox("StripS",pMedSi,dx/2.,dy/2.,dz/2.);
  StripS->SetVisLeaves(kTRUE);
  StripS->SetLineColor(19);

  dx = 7.040;
  dy = 0.0104;
  dz = 0.0150;
  TGeoVolume *StripK = gGeoManager->MakeBox("StripK",pMedSi,dx/2.,dy/2.,dz/2.);
  StripK->SetVisLeaves(kTRUE);
  StripK->SetLineColor(14);

  //Loops to create one AMS detector
  //S-side
  for(Int_t i=0;i<640;i++){
   ...
   AmsDet->AddNode(StripS,i,pMatrix);
  }
  ...
  for(Int_t i=0;i<384;i++){
   dx = 0.000;
   //std::cout<< dx<<std::endl;
   dy = -3.9936/2.0+0.0104/2.0+0.0104*i;
   dz = 0.0150/2.0;
   TGeoCombiTrans* pMatrix = new TGeoCombiTrans("",dx,dy,dz,fRefRot);
   AmsDet->AddNode(StripK,i,pMatrix);
  }
  ...
  pWorld->AddNode(AmsDet,0,pMatrixd0);
  ...
  pWorld->AddNode(AmsDet,1,pMatrixd1);
  ...
  pWorld->AddNode(AmsDet,2,pMatrixd2);
  ...
  pWorld->AddNode(AmsDet,3,pMatrixd3);
  ...
  pWorld->AddNode(AmsDet,4,pMatrixd4);
