...
void ConstructGeometrySOFIAp2p(TGeoMedium *pMed1, TGeoMedium *pMed2);



void create_target_geo(const char* geoTag = "SofH2")
{
  ...
  TString geoFileName = geoPath + "/sofia/geometry/target_";
  ...
  // p2p sofia
  if (0 == strncmp(geoTag, "SofH2", 5))    ConstructGeometrySOFIAp2p(pMed,pMed3);
...
void ConstructGeometrySOFIAp2p(TGeoMedium *pMed1, TGeoMedium *pMed2)
{
// Created for SOFIA experiments with p2p
// joseluis.rodriguez.sanchez@usc.es
// 24/11/2017
//  
  Double_t Length = 1.2;// total in cm

  cout << endl;
  cout << "-I- R3BTarget:: ConstructGeometry() "<< endl;
  cout << "-I- R3BTarget Target type: H2 for p2p with SOFIA, length: " << Length << endl;
  cout << endl;
  ...
  dz   = 3.;           //1.750000;
  TGeoShape *pTarget1 = new TGeoTube("Target1",rmin,rmax,dz);
  TGeoVolume *pTarget1_log = new TGeoVolume("Target1",pTarget1, pMed2);   // Mylar
  pTarget1_log->SetVisLeaves(kTRUE);
  //pTargetEnveloppe_log->AddNode(pTarget1_log, 0, pMatrix4);
  ...
  dz   = Length/2.;     
  TGeoShape *pTarget2 = new TGeoTube("Target2",rmin,rmax,dz);
  TGeoVolume *pTarget2_log = new TGeoVolume("Target2",pTarget2, pMed2);   // H2
  pTarget2_log->SetVisLeaves(kTRUE);
  pTargetEnveloppe_log->AddNode(pTarget2_log, 0, pMatrix6);
  ...
  dz   = 0.00500; // =half thickness for Mylar
  TGeoShape *pTarget3 = new TGeoTube("Target3",rmin,rmax,dz);
  TGeoVolume *pTarget3_log = new TGeoVolume("Target3",pTarget3, pMed2);    // Mylar
  pTarget3_log->SetVisLeaves(kTRUE);
  //pTargetEnveloppe_log->AddNode(pTarget3_log, 0, pMatrix8);
  //pTargetEnveloppe_log->AddNode(pTarget3_log, 1, pMatrix10);
  ...
  phi2 = 360.000000;
  TGeoShape *pTarget4 = new TGeoCone("Target4",dz,rmin,rmax,rmin2,rmax2);
  TGeoVolume *pTarget4_log = new TGeoVolume("Target4",pTarget4, pMed2);    // Gold
 // pTarget4_log->SetVisLeaves(kTRUE);
//gTop->AddNode(pTarget4_log, 0, pMatrix12);
//pWorld->AddNode(pTarget4_log, 0, pMatrix12);
  ...
  dy = 0.000000;
  dz = 0.000000;
  TGeoCombiTrans *pMatrix = new TGeoCombiTrans("", dx,dy,dz,0);

  // Shape: TargetH2 type: TGeoTubeSeg
  // H2 Tube 1
  rmin = 0.0;
  rmax = 3.5;              //
  dz   = Length/2.0;       //
  TGeoShape *pTargetH2 = new TGeoTube("TargetH2",rmin,rmax,dz);
  TGeoVolume *pTargetEnveloppe_log = new TGeoVolume("TargetEnveloppe",pTargetH2, pMed2); // H2
  pTargetEnveloppe_log->SetVisLeaves(kTRUE);
 ...
 // TGeoVolumeAssembly *keep = new TGeoVolumeAssembly("Target");
 // keep->AddNode(pTargetH2_log, 0, pMatrix);
