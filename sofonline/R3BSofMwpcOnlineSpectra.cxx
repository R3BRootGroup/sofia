...
R3BSofMwpcOnlineSpectra::R3BSofMwpcOnlineSpectra(const TString& name, Int_t iVerbose, 
...
R3BSofMwpcOnlineSpectra::~R3BSofMwpcOnlineSpectra() {
  ...
  if(fCalItemsMwpc) delete fCalItemsMwpc;
}
...
InitStatus R3BSofMwpcOnlineSpectra::Init() {
  ...
  if (!fCalItemsMwpc) { return kFATAL;}
  ...
  run->GetHttpServer()->RegisterCommand("Reset_"+fNameDet+"_HIST", Form("/Objects/%s/->Reset_Histo()", GetName()));
...
void R3BSofMwpcOnlineSpectra::Exec(Option_t* option) {
  ...
  if(fCalItemsMwpc && fCalItemsMwpc->GetEntriesFast()){
	...
	(R3BSofMwpcCalData*)fCalItemsMwpc->At(ihit);
      if (!hit) continue;
        if(hit->GetPlane()==1){
    ...
    if(maxpadx>-1&&maxpady>-1)fh2_mwpc_cal->Fill(maxpadx,maxpady);
  }
