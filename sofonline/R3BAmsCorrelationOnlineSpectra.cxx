...
InitStatus R3BAmsCorrelationOnlineSpectra::Init()
{
    ...
    if (!fHitItemsAms) {
        LOG(warn) << "R3BAmsCorrelationOnlineSpectra: AmsHitData not found";
        // return kFATAL;
    }
    ...
    if (!fHitItemsMus)
        LOG(warn) << "R3BAmsCorrelationOnlineSpectra: MusicHitData not found";

    // get access to hit data of the Triple-MUSIC
    fHitItemsTrim = (TClonesArray*)mgr->GetObject("TrimHitData");
    if (!fHitItemsTrim)
        LOG(warn) << "R3BAmsCorrelationOnlineSpectra: TrimHitData not found";
    ...
    if (tpat == 4 && z_twim > 90. && z_twim < 92.) {
        ...
        if (fHitItemsCalifa && fHitItemsCalifa->GetEntriesFast() > 0) {
            ...
            for (Int_t i1 = 0; i1 < nHits; i1++) {
                ...
                if (califa_e[i1] > maxEL && TMath::Abs(califa_phi[i1]) < 60.) {   // messel
