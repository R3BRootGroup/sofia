...
InitStatus R3BSofToFWOnlineSpectra::Init()
{
    ...
    for(Int_t i=0; i<NbDets; i++)    mainfolToFW->Add(cToFWRawTof[i]);
...
void R3BSofToFWOnlineSpectra::Exec(Option_t* option)
{
    ...
    if (fMappedItemsToFW && fMappedItemsToFW->GetEntriesFast() &&
	fTcalItemsToFW   && fTcalItemsToFW->GetEntriesFast()){
	...
	if((mult[iDet*NbChs]==1)&&(mult[iDet*NbChs+1]==1)){
	  ...
	  if(TrawStart != -1000000.){
	    fh1_RawTof_AtTcalMult1[i]->Fill((Double_t)(0.5*(iRawTimeNs[i*2]+iRawTimeNs[i*2+1]))-TrawStart);
	  }
