...
InitStatus R3BSofTofWOnlineSpectra::Init()
{
    ...
    if (!fSingleTcalItemsTofW)
    {
        LOG(warn) << "R3BSofTofWOnlineSpectra: SofTofWSingleTcalData not found";
    }
    ...
    if (!fHitItemsTofW)
    {
        LOG(warn) << "R3BSofTofWOnlineSpectra: TofWHitData not found";
    }
    ...
    if (!fHitItemsTwim)
        LOG(warn) << "R3BSofTofWOnlineSpectra: TwimHitData not found";

    // get access to cal data of the MWPC3
    fCalItemsMwpc = (TClonesArray*)mgr->GetObject("Mwpc3CalData");
    if (!fCalItemsMwpc)
        LOG(warn) << "R3BSofTofWOnlineSpectra: Mwpc3CalData not found";
