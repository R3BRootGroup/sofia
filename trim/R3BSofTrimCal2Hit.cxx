...
R3BSofTrimCal2Hit::~R3BSofTrimCal2Hit()
{
    ...
    if (fTrimCalData)  delete fTrimCalData;
    if (fTrimHitData)  delete fTrimHitData;
...
void R3BSofTrimCal2Hit::Exec(Option_t* option)
{
    ...
    Int_t nAligned, nRaw;
    if (fTriShape==kTRUE) nAligned=fNumAnodes/2;
    else                  nAligned=fNumAnodes; 
	...
	dt[a+s*fNumAnodes] = -1000000.;
      ...
      for(Int_t ch=0; ch<nAligned; ch++) eal[ch+s*nAligned]=0;
    }
	...
	for(Int_t ch=0; ch<nAligned; ch++){
	  if(mult[2*ch+s*fNumAnodes]==1 && mult[2*ch+1+s*fNumAnodes]==1){
	    eal[ch+s*nAligned] = (e[2*ch+s*fNumAnodes]+e[2*ch+1+s*fNumAnodes])*fTrimHitPar->GetEnergyAlignGain(s+1, ch);
	    sumRaw += eal[ch+s*nAligned];
	    nRaw++;
	  }	
	}// end of loop over the anodes
      }// end of calculation of fEnergyRaw for triangular shape anodes
      if(nRaw>0) sumRaw = sumRaw/(Float_t)nRaw;
...
void R3BSofTrimCal2Hit::Reset()
{
    ...
    if (fTrimHitData) fTrimHitData->Clear();
}

// -----   Private method AddCalData  --------------------------------------------
R3BSofTrimHitData* R3BSofTrimCal2Hit::AddHitData(Int_t   secID,  Float_t Eraw, 
