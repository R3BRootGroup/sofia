...
class R3BSofTrimHitPar : public FairParGenericSet
{
  ...
  public:
    ...
    Float_t GetEnergyAlignGain(Int_t section, Int_t index)
    {
        return fEnergyAlignGains->GetAt((section - 1) * fNumAlignGainsPerSection + index);
    } // index is 0-based
    ...
    void SetEnergyAlignGain(Float_t val, Int_t section, Int_t index)
    {
        fEnergyAlignGains->AddAt(val, (section - 1) * fNumAlignGainsPerSection + index);
    }
  ...
  private:
    ...
    Int_t fNumCorrBetaParsPerSection; // if pol2 -> 3 parameters (P0, P1, P2) P0 at degree=0, P1 at degree=1, P2 at
                                      // degree=2
