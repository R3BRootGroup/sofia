...
Bool_t R3BSofTrimReader::Init(ext_data_struct_info* a_struct_info)
{
    ...
    for (int s = 0; s < NbSections_SofTrim; s++)
    {
...
Bool_t R3BSofTrimReader::Read()
{
    ...
    for (UShort_t s = 0; s < NbSections_SofTrim; s++)
    {
        ReadData(data, s);
    }
    return kTRUE;
...
Bool_t R3BSofTrimReader::ReadData(EXT_STR_h101_SOFTRIM_onion* data, UShort_t section)
{
  ...
  for (UShort_t a = 0; a < nAnodesTref; a++){
	...
	// section is 0-based
	new ((*fArray)[fArray->GetEntriesFast()]) R3BSofTrimMappedData(section+1, idAnodeTref, data->SOFTRIM_S[section].TREFv[hit], 0, pileupFLAG, overflowFLAG);
	if(section==0)
	  new ((*fArray)[fArray->GetEntriesFast()]) R3BSofTrimMappedData(section+2, idAnodeTref, data->SOFTRIM_S[section].TREFv[hit], 0, pileupFLAG, overflowFLAG);
      }
      curTref = nextTref;
    }
	...
	new ((*fArray)[fArray->GetEntriesFast()]) R3BSofTrimMappedData(section+1, idAnodeTtrig, data->SOFTRIM_S[section].TTRIGv[hit], 0, pileupFLAG, overflowFLAG);
	if(section==0)
	  new ((*fArray)[fArray->GetEntriesFast()]) R3BSofTrimMappedData(section+2, idAnodeTtrig, data->SOFTRIM_S[section].TTRIGv[hit], 0, pileupFLAG, overflowFLAG);
      }
      curTtrig = nextTtrig;
    }
