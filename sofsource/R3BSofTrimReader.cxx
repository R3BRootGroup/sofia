...
Bool_t R3BSofTrimReader::Init(ext_data_struct_info* a_struct_info)
{
    ...
    for (int mult = 0; mult < NUM_SOFTRIM_ANODES; mult++)
        multPerAnode[mult] = 0;
...
Bool_t R3BSofTrimReader::ReadData(EXT_STR_h101_SOFTRIM_onion* data, UShort_t section)
{
    ...
    if (nTref > 1)
        LOG(ERROR) << "R3BSofTrimReader::ReadData ERROR ! ONLY ONE Tref signal PER SECTION BUT HERE MULTIPLE SIGNALS "
                      "IS FOUND!";
    uint32_t curTref = 0;
    uint32_t nextTref;
    if (nTref == 1)
    {
        ...
        for (int hit = curTref; hit < nextTref; hit++)
            R3BSofTrimMappedData* mapped = new ((*fArray)[fArray->GetEntriesFast()])
                R3BSofTrimMappedData(section + 1, 7, data->SOFTRIM_S[section].TREFv[hit], 0);
    }
    ...
    for (UShort_t a = 0; a < nAnodesTime; a++)
    {
        ...
        {
            R3BSofTrimMappedData* mapped = new ((*fArray)[fArray->GetEntriesFast()]) R3BSofTrimMappedData(
                section + 1, idAnodeEnergy, data->SOFTRIM_S[section].Tv[hit], data->SOFTRIM_S[section].Ev[hit]);
        }
