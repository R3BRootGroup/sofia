...
Bool_t R3BSofComRefReader::Read()
{
    ...
    */
    ...
    for (int d = 0; d < NUM_SOFCOMREF_SIGNALS; d++)
    {
        ...
        if ((TotalNumberOfHits == 1) && (NumberOfSignalWithHits == TotalNumberOfHits))
            FLAG_SIGNAL = kTRUE;
        ...
        for (int signalmult = 0; signalmult < NumberOfSignalWithHits; signalmult++)
        {
            for (int hit = 0; hit < data->SOFCOMREF_S[d].TFME[0]; hit++)
                new ((*fArray)[fArray->GetEntriesFast()]) R3BSofComRefMappedData(
                    d + 1, data->SOFCOMREF_S[d].TCv[hit], data->SOFCOMREF_S[d].TFv[hit], FLAG_SIGNAL);
        }
    }
    return kTRUE;
}
