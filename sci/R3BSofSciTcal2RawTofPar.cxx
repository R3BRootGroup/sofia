...
void R3BSofSciTcal2RawTofPar::Exec(Option_t* opt)
{
    ...
    for (UShort_t dstart = 0; dstart < NUMBER_OF_SOFSCI_DETECTORS - 1; dstart++)
    {
        ...
        {
            iTrawStart = 0.5 * (iRawTimeNs[dstart * NUMBER_OF_SOFSCI_CHANNELS] +
                                iRawTimeNs[dstart * NUMBER_OF_SOFSCI_CHANNELS + 1]);
            iTrawStop = 0.5 * (iRawTimeNs[dstop * NUMBER_OF_SOFSCI_CHANNELS] +
                               iRawTimeNs[dstop * NUMBER_OF_SOFSCI_CHANNELS + 1]);
            fh_RawTofMult1[dstart]->Fill(iTrawStop - iTrawStart + iRawTimeNs[dstart * NUMBER_OF_SOFSCI_CHANNELS + 2] -
                                         iRawTimeNs[dstop * NUMBER_OF_SOFSCI_CHANNELS + 2]);
        }
    }
}
...
void R3BSofSciTcal2RawTofPar::CalculateRawTofParams()
{
    ...
    for (Int_t sig = 0; sig < fNumSignals; sig++)
    {
        ...
        if (fh_RawTofMult1[sig]->GetEntries() > fMinStatistics && iMax > 0)
        {
            ...
            {
                if (fh_RawTofMult1[sig]->GetBinContent(bin) > (iMax / 1000.))
                    binLimit = bin;
                bin++;
            }
