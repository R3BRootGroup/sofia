...
InitStatus R3BSofSciTcal2SingleTcal::Init()
{
    ...
    if(fRawTofPar->GetDetIdCaveC()!=1 && fRawTofPar->GetNumDets()==1) {
      LOG(ERROR) << "Number of SofSci declared to 1 (primary beam), but idCave = " << fRawTofPar->GetDetIdCaveC() << " instead of 1";
      return kFATAL;
    }

    if(fRawTofPar->GetDetIdCaveC()!=fRawTofPar->GetNumDets() && fRawTofPar->GetNumDets()>1) {
      LOG(ERROR) << "Number of SofSci declared >1 (secondary beam), but idCave = " << fRawTofPar->GetDetIdCaveC() << " instead of " << fRawTofPar->GetNumDets();
      return kFATAL;
    }
...
void R3BSofSciTcal2SingleTcal::Exec(Option_t* option)
{
  ...
  if ((nHitsPerEvent_SofSci > 0) && (mult[(idCaveC-1)*nChs] > 0) && (mult[(idCaveC-1)*nChs+1] > 0)){
	...
	for (UShort_t multL=0; multL<mult[1]; multL++){
	  ...
	  if (iRawPos > fRawPosPar->GetSignalTcalParams(1)) continue;
	  // if the left or right hit has already been used, continue
          if ((((maskR[0] >> multR) & (0x1)) == 1) || (((maskL[0] > multL) & (0x1)) == 1)) continue;
...
R3BSofSciSingleTcalData* R3BSofSciTcal2SingleTcal::AddSingleTcalData(UShort_t iDet, Double_t traw, Double_t posraw, Double_t tofrawS2, Double_t tofrawS8)
{
